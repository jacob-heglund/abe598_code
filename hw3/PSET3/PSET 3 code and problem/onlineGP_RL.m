...
%             BV(a,1) = data(1:params.N_state_dim);
%             rew(a,:)=reward;
%             y= reward+params.gamma*Qmax;
%             obs=y;
%             current_size(a) = size(data,2);
%             K(a,:) = kernel(data,data,sigma,params);
%             K_red=getactivebasis(K(a,:),a,params);%K(((a-1)*params.N_phi_s + 1):a*params.N_phi_s);%extract the basis
%             Q(a,1,:) = inv(K_red); % inverted gram matrix
%             K_red = K_red + noise*eye(current_size(a));
%             C(a,1,:) = inv(K_red); % less numerically stable than cholesky
%             alpha_temp = C(a,1,:)*y';%find the weights
%             alpha = zeros(1,params.N_phi_s*params.N_act,1);
%             alpha(1,((a-1)*params.N_phi_s + 1):a*params.N_phi_s) = alpha_temp;%update the weights only wrt to the current action           
        end
...
%             kxx=getactivebasis(kxx,a,params);
            var_x=kxx+ kxx'*C(1:current_size(index),1:current_size(index))*kxx;%needs to be sent out for each action
          %  [BV_store,K_store,alpha_store,C_store,Q_store,current_size_store,obs_store,rew_store,index_store]=assign_vars_back(a,BV_store,K_store,alpha_store,C_store,Q_store,current_size_store,obs_store,rew_store,index_store);
        end
    end
